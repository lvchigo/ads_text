...
/***********************************CountLines***********************************/
int API_DOC2VEC::doc2vec_CountLines(char *filename)
{
	ifstream ReadFile;
	int n=0;
	char line[4096];
...
int API_DOC2VEC::Init( const char *Keyfile )
{
	...
	string word;
	float tmp, idf = 0;
	long long Count, i, j, a, b, w2v_words, key_words, w2v_size;
	UInt64 code = 0;
	
	FILE *fpW2VList = 0, *fpIDFList = 0, *fpStopWord = 0, *fpSensitiveWord = 0, *fpKeyWord = 0;

	/*****************************Init:Cutword*****************************/
	map< string,int >::iterator 				itStopwords;
	map< UInt64, vector< string > >::iterator 	itDicCode;		//[In]dic word
	map< string, int >::iterator 				itKeyWord;		//[In]key word by people
	...
	sprintf(loadFile, "%s/w2v/alldata_20151013/liblinear_adstext2class_2015102003.model",Keyfile);	//linearSVM
	...
	/*****************************Load SensitiveWords*****************************/
	sprintf(loadFile, "%s/w2v/alldata_20151013/in_ads_SensitiveWords.csv",Keyfile);
	...
	/*****************************Load keyword.txt*****************************/
	sprintf(loadFile, "%s/w2v/alldata_20151013/ads_keyword_151021.txt",Keyfile);
	printf("Load keyword.txt:%s\n",loadFile);

	fpKeyWord = fopen(loadFile,"r");
	if (!fpKeyWord) 
	{
		cout << "Can't open " << loadFile << endl;
		return -1;
	...
	Count = 0;
	mapKeyWord.clear();
	while(EOF != fscanf(fpKeyWord, "%s %s", &loadPath, &charIdf ))
	{
		word = loadPath;
		label = atoi(charIdf);
		if ( (word == " ") || (word == "	") || ( label<1 ) || ( label>4 ) )
			continue;
		else
			mapKeyWord[word] = label;
	...
	printf("mapKeyWord.size():%ld\n",mapKeyWord.size());

	//judge
	if ( mapKeyWord.size() == 0 )
	{
		printf("mapKeyWord.size() == 0\n");
		return -1;
	}

	//write mapKeyDicWord
	mapKeyDicWord.clear();		//[In]key word in dic
	for(itKeyWord = mapKeyWord.begin(); itKeyWord != mapKeyWord.end(); itKeyWord++)
	{
		if ( ( itKeyWord->second == 1 ) || ( itKeyWord->second == 2 ) ) //Use 1st&&2nd Word
		{
			itDicWord = mapDicWord.find(itKeyWord->first);		
			if (itDicWord != mapDicWord.end()) // find it
			{
				mapKeyDicWord[itKeyWord->first] = itDicWord->second;		//[In]dic code-words
			}
		}
		//printf("word:%s,label:%d\n",itKeyWord->first.c_str(),itKeyWord->second);
	}
	printf( "mapKeyWord.size()=%d,mapKeyDicWord.size()=%d\n", mapKeyWord.size(), mapKeyDicWord.size() );
	...
	vector< string > 	&vecFilterWord )	//[Out]filter word
{
	...
	for(i=0;i<vecQuaryWord.size();i++)
	{
		...
		//catch(const boost::bad_exdivssion &e)
		{
			printf("Err String:%s\n",testString.c_str());
			std::cout <<"That's not a valid regular exdivssion! (Error: " <<e.what() << ") Exiting.../n";
			continue;
		}
	}

	if ( vecFilterWord.size() == 0 )
	{
		//printf( "API_DOC2VEC::FilterQuaryString err!!vecFilterWord.size:%ld!!\n", vecFilterWord.size() );
		return -1; 
	}	
...
int API_DOC2VEC::Predict_ExpertRule( 
	vector< string > 	vecQuaryWord,		//[In]quary word
	vector< string > 	&vecFilterWord )	//[Out]filter word
{
	if ( vecQuaryWord.size() == 0 )
...
int API_DOC2VEC::Predict_shortText(
	vector< string > 										vecQuaryWord,	//[In]quary word
	...
	map< vector< string >, vector< pair< int, float > > > 	&resPredict )	//[Out]Res:string-useful words,int-label,float-score
{
	...
	{	
		ErrPredict(resPredict);
	   	//cout<<"Fail to Predict:FilterQuaryString !! "<<endl; 
	   	return 0;
	}
	...
	{
		PredictFeat.push_back( itPCAFeat->second );
		for(i=0;i<itPCAFeat->first.size();i++)
			vecCutWord.push_back(itPCAFeat->first[i]);
	}
	...
	resSimWord.clear();
 	nRet = FindSimilarWord_shortText( vecCutWord, resSimWord );
	if ( nRet != 0 )
	{	
	   	//cout<<"Fail to FindSimilarWord_shortText!! "<<endl; 
	   	return nRet;
	}	

	/************************Merge*****************************/	
	resPredict.clear();
	nRet = Merge_shortText( inMergePredict, queryStringSize, resSimWord, resPredict );
	if ( nRet != 0 )
	{	
	   	cout<<"Fail to Predict SVM!! "<<endl; 
	   	return nRet;
	...
	map< vector< string >, vector< pair< int, float > > > 	&resPredict )	//[Out]Res:string-useful words,int-label,float-score
{
	...
	/************************Predict_ExpertRule*****************************/	
/*	vecFilterNumber.clear();
	nRet = Predict_ExpertRule( vecFilterWord, vecFilterNumber );
	if ( nRet != 0 )
	{	
		//cout<<"Fail to Predict_ExpertRule!! "<<endl; 
		return nRet;
	}	*/
	...
	labelExpertRule = 0;
	resPredict.clear();
	nRet = Merge_2( inMergePredict, queryStringSize, resSimWord, vecFilterNumber, labelExpertRule, resPredict );
	if ( nRet != 0 )
	{	
	   	cout<<"Fail to Predict SVM!! "<<endl; 
	   	return nRet;
...
int API_DOC2VEC::Merge_shortText(
	map< vector< string >, vector< pair< int, float > > > 	inPredict,
	long 													queryStringSize,
	...
	map< vector< string >, vector< pair< int, float > > > 	&outPredict)
{
	...
	for(itInPredict = inPredict.begin(); itInPredict != inPredict.end(); itInPredict++)
	{
		if ( itInPredict->first.size()>2 )
		//if ( ( itInPredict->first.size()>2 ) && ( ( itInPredict->second[0].second >=0.99995 ) || ( queryStringSize>=45 ) ) )
		//if ( ( itInPredict->first.size()>2 ) && ( queryStringSize>=45 ) )
		{
			outPredict[itInPredict->first] = itInPredict->second;
		}
	...
	map< vector< string >, vector< pair< int, float > > > 	&outPredict)
{
	...
	/*****************************Init*****************************/
	int nlabel,labelKeyWord,nRet = 0;
	long long i,nCount_1,nCount_other,nCount_all;
	map< vector< string >, vector< pair< int, float > > >::iterator itInPredict;
	...
	for(itInPredict = inPredict.begin(); itInPredict != inPredict.end(); itInPredict++)
	{
		...
		if ( ( itInPredict->first.size()>2 ) && ( queryStringSize>=45 ) && 
			 ( itInPredict->second[0].first == 1 ) && ( itInPredict->second[0].second >=0.8 ) )
		{
			outPredict[itInPredict->first] = itInPredict->second;
		}
		else if ( itInPredict->first.size()>1 )
		{
			/************************count SimWord*****************************/
			nCount_1 = 0;
			nCount_other = 0;
			...
			{
				labelKeyWord = 0;
				itKeyWord = mapKeyWord.find(itSimWord->second[0].first);		
				if (itKeyWord != mapKeyWord.end()) // find it
				{
					labelKeyWord = itKeyWord->second;
				}
		
				if ( ( itSimWord->second[0].second >= 0.9999 ) && ( labelKeyWord == 1 ) )		//1st word
					nCount_1++;
				else if ( ( itSimWord->second[0].second >= 0.9 ) && 
					( ( labelKeyWord == 1 ) || ( labelKeyWord == 2 ) ) )						//1st && 2nd word
					nCount_other++;
			}

			nCount_all = nCount_1 + nCount_other;

			
			if ( ( itInPredict->second[0].first == 0 ) && ( itInPredict->second[0].second >= 0.9999 ) )	//neg-max
			{
				outPredict[itInPredict->first] = itInPredict->second;
			}
			else if ( ( ( nCount_1>0 ) || ( nCount_all>1 ) ) && (itInPredict->second[0].first == 1) )	//pos-all
			{
				for (i=0;i<itInPredict->second.size();i++)
				{
					vecRes.push_back( std::make_pair( 2, 1+itInPredict->second[i].second ) );
				}
				outPredict[itInPredict->first] = vecRes;
			}
			else if ( nCount_all>1 )																	//neg-all
			{
				for (i=0;i<itInPredict->second.size();i++)
				{
					vecRes.push_back( std::make_pair( 2, itInPredict->second[i].second ) );
				}
				outPredict[itInPredict->first] = vecRes;
			}
			else if ( ( itInPredict->first.size()>2 ) && ( itInPredict->second[0].first == 1 ) &&
						( queryStringSize>=45 ) )														//pos>45
			{
				outPredict[itInPredict->first] = itInPredict->second;
			}
			else if ( ( itInPredict->first.size()>2 ) && ( itInPredict->second[0].first == 1 ) )		//pos.other
			{
				for (i=0;i<itInPredict->second.size();i++)
				{
					vecRes.push_back( std::make_pair( -1, itInPredict->second[i].second ) );
				}
				outPredict[itInPredict->first] = vecRes;
			}
			else if ( ( itInPredict->first.size()>2 ) && ( itInPredict->second[0].first == 0 ) )		//neg.other
			{
				outPredict[itInPredict->first] = itInPredict->second;
			}
			else																						//other>1
...
int API_DOC2VEC::FindSimilarWord_shortText(
	vector< string > 									vecQuaryWord,	//[In]quary word
	map< string , vector< pair< string, float > > > 	&resSimWord)	//[Out]Res:string-useful words,int-label,float-score
{
	...
	map< string, vector< float > >::iterator 		itDicWord;		//[In]dic word
	map< string, float >::iterator 					itDicWordDist;	//[In]dic word distance
	map< string, vector< float > >::iterator 		itKeyDicWord;	//[In]dic word
	...
	for(itQuaryWord = mapQuaryWord.begin(); itQuaryWord != mapQuaryWord.end(); itQuaryWord++)
	{
		...
		//find queryDist
		queryDist = 0;
		itDicWordDist = mapDicWordDist.find(wordQuary);
		if (itDicWordDist != mapDicWordDist.end()) // find
			queryDist = itDicWordDist->second;
		else
			continue;

		/************************Distance_Cosine*****************************/
		distQueryCode.clear();
		for(itKeyDicWord = mapKeyDicWord.begin(); itKeyDicWord != mapKeyDicWord.end(); itKeyDicWord++)
		{
			//dic info
			vecDic.clear();
			wordDic = itKeyDicWord->first;
			vecDic = itKeyDicWord->second;
		...
		res.clear();
		res.assign( distQueryCode.begin(), distQueryCode.begin()+3 );
...
void API_DOC2VEC::Release()
{
	...
	mapKeyWord.clear();		//[In]key word by people	[use]search word
	mapKeyDicWord.clear();	//[In]key word in dic		[use]search sim word
}
