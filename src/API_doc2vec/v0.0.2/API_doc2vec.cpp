...
/*
CppJieba::Application app( "/home/xiaogao/job/word2vec/keyfile/cutword-geiba/jieba.dict.utf8",
...
/***********************************ImgSortComp*************************************/
static bool ImgSortComp(const pair< unsigned long long, int > elem1, const pair< unsigned long long, int > elem2)
{
...
int API_DOC2VEC::Init( const char *Keyfile )
{
	...
	/***********************************Init api_pca*************************************/
	sprintf(loadFile, "%s/w2v/pca_text2class_050828.model",Keyfile);
	printf("load api_pca:%s\n",loadFile);
	api_pca.PCA_Feat_Init(loadFile); 

	/***********************************Init Model_SVM*************************************/
	sprintf(loadFile, "%s/w2v/liblinear_text2class_050828_2000.model",Keyfile);	//linearSVM
	...
	for (b = 0; b < w2v_words; b++) {
		...
		//save data
		mapDicWord[word] = NormVec;		//[In]dic word
		mapDicWordDist[word] = tmp;		//[In]dic word distance
		mapDicIndex[word] = b;			//[In]dic word Index
		nRet = api_commen.EncodeFeature( NormVec, code );	//encode feat
		if ( nRet != 0 ) 
		{
	...
	map< vector< string >, vector< float > > 		&mapQuaryDoc )	//[Out]quary Doc
{
	...
	map< UInt64, vector< string > >::iterator 	itDicCode;		//[In]dic word
	map< vector< string >, vector< string > >::iterator 	itQuaryDocString;
	map< string, long >::iterator 				itDicIndex;		//[In]dic word Index
	
	/*****************************word2doc*****************************/
	for(itQueryCode = mapQuaryCode.begin(); itQueryCode != mapQuaryCode.end(); itQueryCode++)
	{
		vector< pair< UInt64, int > > distQueryCode; //every query 
		for(itDicCode = mapDicCode.begin(); itDicCode != mapDicCode.end(); itDicCode++)
		{
			distance = api_commen.Distance_Hamming( itQueryCode->first, itDicCode->first );
		...
		{
			//save string
			itQuaryDocString = mapQuaryDocString.find(itQueryCode->second);		
			itDicCode = mapDicCode.find(distQueryCode[i].first);	
			if ( (itQuaryDocString != mapQuaryDocString.end()) && (itDicCode != mapDicCode.end()) ) // find it	
				itQuaryDocString->second.assign( itDicCode->second.begin(), itDicCode->second.end() );
			else if ( (itQuaryDocString == mapQuaryDocString.end()) && (itDicCode != mapDicCode.end()) )	
				mapQuaryDocString[itQueryCode->second] = itDicCode->second;

			//query strings Use 1000 doc strings
			sumStringCount += distQueryCode[i].second;
			if ( sumStringCount>1000 )
				break;
		}
	}

	for(itQuaryDocString = mapQuaryDocString.begin(); itQuaryDocString != mapQuaryDocString.end(); itQuaryDocString++)
	{
		vecQueryString.clear();
		vecDocString.clear();
		vecQueryString = itQuaryDocString->first;
		vecDocString = itQuaryDocString->second;

		for(i=0;i<vecDocString.size();i++)
		{
			//dic info
			vecDic.clear();
			wordDic = vecDocString[i];
			...
			//dic index
			itDicIndex = mapDicIndex.find(wordDic);
			if (itDicIndex != mapDicIndex.end()) // find it
				indexDic = itDicIndex->second;
			else
				continue;

			//Read Quary Info
			docFeat = 0;
			for(j=0;j<vecQueryString.size();j++)
			{
				//quary info
				vecQuary.clear();
				wordQuary = vecQueryString[j];
				itQuaryWord = mapQuaryWord.find(wordQuary);
				if (itQuaryWord != mapQuaryWord.end()) // find it
				...
				if ( docFeat > vecDocFeat[indexDic] )
					vecDocFeat[indexDic] = docFeat;	//get max value
			}
		}
	}
...
/*	//check info
	...
	for (i = 0; i < outQueryWords.size(); i++) 
	{
		...
		//Quary Vector && Quary code
		itDicWord = mapDicWord.find(word);
		if (itDicWord != mapDicWord.end()) // find it
		{
			mapQuaryWord[word] = itDicWord->second;

			nRet = api_commen.EncodeFeature( itDicWord->second, code );	//encode feat
			if ( nRet != 0 ) 
			{
				printf("doc2vec:EncodeFeature err!!\n");
				continue;
			}
			itQueryCode = mapQuaryCode.find(code);		
			if (itQueryCode != mapQuaryCode.end()) // find it
				itQueryCode->second.push_back(word);
			else
			{
				codeString.clear();
				codeString.push_back(word);
				mapQuaryCode[code] = codeString;
			}
		}
		else
			continue;
	...
	//word2doc
	mapQuaryDoc.clear();
	nRet = doc2vec_word2doc( mapQuaryWord, mapQuaryTf, mapQuaryCode, mapQuaryDoc );
	if ( nRet != 0 ) 
	{
