...
int cutword_GetSample( char *szQueryList, char *poskeyword, char *negkeyword, char *szResList )
{
	/*****************************Init*****************************/
	char loadPath[4096];
	char posAddChar[1024];
	int nRet = 0;
	long nCount;
	std::size_t found;
	FILE *fpListFile = 0, *fpPos = 0, *fpNeg = 0, *fpOut = 0;

	API_DOC2VEC api_doc2vec;

	char tmpChar;
	char vocab[256];
	string word,s,result,posAddString;
	long long idQuary, labelQuary, labelpos, labelneg,  i, j, a, b, w2v_words, w2v_size;
	
	/*****************************Init*****************************/
	map< string,int > mapPosKeyword;
	map< string,int > mapNegKeyword;
	map< string,int >::iterator itPosKeyword;
	map< string,int >::iterator itNegKeyword;
	...
	fpOut = fopen(szResList, "wt");
	if (!fpOut)
	{
		cout << "Can't open fpOut file " << szResList << endl;
		return -1;
	}

	/*****************************Load:PosKeyword*****************************/
	printf("Load poskeyword:%s\n",poskeyword);
	fpPos = fopen(poskeyword,"r");
	if (!fpPos) 
	{
		cout << "Can't open " << poskeyword << endl;
		return -1;
	}

	mapPosKeyword.clear();
	while(EOF != fscanf(fpPos, "%s", &loadPath ))
	{
		word = loadPath;		
		if ( word == " " )
			continue;

		itPosKeyword = mapPosKeyword.find(word);
		if (itPosKeyword == mapPosKeyword.end()) // not find
		{
			mapPosKeyword[word] = 1;
		}
	}

	/*****************************Load:NegKeyword*****************************/
	printf("Load negkeyword:%s\n",negkeyword);
	fpNeg = fopen(negkeyword,"r");
	if (!fpNeg) 
	{
		cout << "Can't open " << negkeyword << endl;
		return -1;
	}

	mapNegKeyword.clear();
	while(EOF != fscanf(fpNeg, "%s", &loadPath ))
	{
		word = loadPath;		
		if ( word == " " )
			continue;

		itNegKeyword = mapNegKeyword.find(word);
		if (itNegKeyword == mapNegKeyword.end()) // not find
		{
			mapNegKeyword[word] = 1;
		}
	}
	...
	for (b = 0; b < w2v_words; b++) {
		...
		{
			//find pos keyword
			for(itPosKeyword = mapPosKeyword.begin(); itPosKeyword != mapPosKeyword.end(); itPosKeyword++)
			{
				sprintf(posAddChar, "\u4e0d%s",itPosKeyword->first.c_str());	//add "no"
				posAddString= posAddChar;
				found = vecQuaryString[i].find(posAddString);
				if (found!=std::string::npos)	//find
				{
					labelneg++;
					//printf( "pos:%s\n",itPosKeyword->first.c_str() );
					break;
				}

				found = vecQuaryString[i].find(itPosKeyword->first);
				if (found!=std::string::npos)	//find
				{
					labelpos++;
					//printf( "pos:%s\n",itPosKeyword->first.c_str() );
			...
			//find neg keyword
			for(itNegKeyword = mapNegKeyword.begin(); itNegKeyword != mapNegKeyword.end(); itNegKeyword++)
			{
				found = vecQuaryString[i].find(itNegKeyword->first);
				if (found!=std::string::npos)	//find
				{
					labelneg++;
					//printf( "neg:%s\n",itNegKeyword->first.c_str() );
					break;
				}
			}
		}	
		...
		labelQuary = -1;
		if ( (labelpos == 0) && (labelneg > 0) ) // bad
			labelQuary = 1;
		else if ( (labelpos > 0 ) && (labelneg > 0 ) ) // special normal
			labelQuary = 4;
		else if ( (labelpos > 0 ) && (labelneg == 0) ) // pos
			labelQuary = 3;
		else if ( (labelpos == 0) && (labelneg == 0) ) // normal
			labelQuary = 2;
	...
	if (fpPos) {fclose(fpPos);fpPos = 0;}
	if (fpNeg) {fclose(fpNeg);fpNeg = 0;}
	...
	for (b = 0; b < w2v_words; b++) {
		...
		for ( i=0;i<MergeWords.size();i++ )
		{
			...
			if ((MergeWords[i].empty()) || (MergeWords[i] == " ") || (MergeWords[i] == "	") || 
				(api_doc2vec.IsDigit2(MergeWords[i])) || (MergeWords[i].size()<4) || (!api_doc2vec.IsAllChinese(MergeWords[i])) )
...
//	vector< string > vecQuaryString;
	...
	for(itWordCount = wordCount.begin(); itWordCount != wordCount.end(); itWordCount++)
	{
		...
		if ( (word.empty()) || (word == " ") || (word == "	") || 
			(api_doc2vec.IsDigit2(word)) || (word.size()<4) || (!api_doc2vec.IsAllChinese(word)) )
			continue;
...
int doc2vec_train( char *szQueryList, char *Keyfile, char *szFeat, int binFeat )
{
	...
	for (b = 0; b < w2v_words; b++) {
		...
		if ( ( QueryAllSize < 45 ) && ( labelQuary == 1) )	//query string.size>=40
		{
...
int doc2vec_findsimword( char *szQueryList, char *Keyfile, char *szResult, int binFeat )
{	
	/*****************************Init:Commen*****************************/
	char loadPath[4096];
	int nRet = 0;
...
{  
	...
	string str = "\u4f60\u597d \u4e16\u754c";  
	printf( "str:%s\n", str.c_str() );
	string temp = api_doc2vec.URL_Encode(str);  
	printf( "temp:%s\n", temp.c_str() );
	string temp2 = api_doc2vec.URL_Decode(temp);
	printf( "temp2:%s\n", temp2.c_str() );

	string str2 = "%E4%BD%A0%E5%A5%BD";
	printf( "str2:%s\n", str2.c_str() );
	string temp3 = api_doc2vec.URL_Decode(str2);
	printf( "temp3:%s\n", temp3.c_str() );
...
int main(int argc, char* argv[])
{
	...
	if (argc == 6 && strcmp(argv[1],"-getsample") == 0)
	{
		ret = cutword_GetSample(argv[2],argv[3],argv[4],argv[5]);
	}
	else if (argc == 5 && strcmp(argv[1],"-getadsstring") == 0)
	{
	...
	else if (argc == 6 && strcmp(argv[1],"-findsimword") == 0)
	{
		ret = doc2vec_findsimword(argv[2],argv[3],argv[4],atol(argv[5]));
	}
	...
	{
		cout << "usage:\n" << endl;
		cout << "\tdemo -getsample List.txt pos.txt neg.txt res.txt\n" << endl;
		...
		cout << "\tdemo -findsimword List.txt Keyfile Result binFeat\n" << endl;
		cout << "\tdemo -test\n" << endl;
		return ret;
	}
	return ret;
